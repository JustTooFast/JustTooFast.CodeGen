...
Add edge case unit tests for each entity in the selected manifest using the matching entity spec.

This transformation adds only edge case tests. It does not add happy path tests or validation tests.
...
Expected spec file:
    ...
    docs/agent-work/specs/csharp/StringLiteral.md
...
2. Read the `## Edge Case Tests` section.
3. Add only the tests listed in that section.
4. Do not use the happy path or validation test sections during this transformation.

If the `## Edge Case Tests` section says `None`:

- do not add edge case tests for that entity
...
Examples:
    ...
    IntegerLiteralEmitterTest.cs
    QualifiedNameEmitterTest.cs

## Expected Starting Structure

Each test file is expected to exist.

If a test file is empty, populate it with the standard test file structure before adding edge case tests:
...
If a test file already contains the standard test class, add edge case tests inside the class body.

## Test Method Pattern

Use the existing MSTest arrange/act/assert pattern:
...
- Add only tests listed under `## Edge Case Tests`.
...
- add the edge case tests from the spec
...
If the spec has no `## Edge Case Tests` section:
...
If an edge case test already exists with the same method name:
...
- add only missing edge case tests when it is safe to do so
...
- skipped entities with no edge case tests
